...
let ReportService = ReportService_1 = class ReportService {
    ...
    buildHtml(r) {
        ...
        const displayAddr = (addr) => addr.length > 12 ? `${addr.slice(0, 6)}…${addr.slice(-4)}` : addr;
        const chainName = (id) => id ? (bridges_1.CHAIN_NAMES[Number(id)] ?? String(id)) : '—';
...
<div class="body">
  ...
  <table>
    ...
    <tbody>
      ${bridges.transfers.slice(0, 15).map((t) => `
...
<div class="footer">
  ...
  Data sourced from Etherscan V2, Relay.link, LI.FI, web3.bio, SocialData
    ...
    riskScore(risk) {
        let score = 0;
        if (risk.crossWalletExits > 0)
            score += risk.crossWalletExits * 10;
        if (risk.unknownExits > 0)
            score += risk.unknownExits * 5;
        if (risk.phishingContact)
            score += 40;
        if (risk.mixerContact)
            score += 40;
        if (risk.highValueBridge)
            score += 20;
        if (score >= 40)
